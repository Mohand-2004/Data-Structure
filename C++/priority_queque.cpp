// Prority Queque Implementation using C++ Programming Language
...
template <typename DataType> struct Item{
    DataType value;
    bool highPriority = false;
};
template <typename DataType> class PriorityQueque{
    private:
        Node<Item<DataType>> linkedlist;
    public:
        ...
        DataType MAX(){
            DataType max = this->linkedlist[0].value;
            for(int i = 0;i<this->linkedlist.lenght();i++){
                if(linkedlist[i].value > max){
                    max = this->linkedlist[i].value;
                }
            }
            return max;
        }
        ...
        DataType MIN(){
            DataType min = this->linkedlist[0].value;
            for(int i = 0;i<this->linkedlist.lenght();i++){
                if(linkedlist[i].value < min){
                    min = this->linkedlist[i].value;
                }
            }
            return min;
        }
        ...
        void enqueue(DataType value,bool high = false){
            Item<DataType> item;
            item.value = value;
        ...
        DataType dequeque(){
            for(int i = 0;i<this->linkedlist.lenght();i++){
                if(this->linkedlist[i].highPriority){
                    return this->linkedlist.pop(i).value;
                }
            }
            return this->linkedlist.pop(0).value;
        }

        // function to print the elements of the queque
        void print(int start = 0,int end = -1){
            if(end == -1){
                end = this->linkedlist.lenght();
            }
            cout<<'[';
            for(int i = start;i<end;i++){
                cout<<this->linkedlist[i].value<<',';
            }
            cout<<']';
        }
